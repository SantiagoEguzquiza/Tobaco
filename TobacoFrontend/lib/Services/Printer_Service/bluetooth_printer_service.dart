...
class BluetoothPrinterService {
  ...
  Future<List<BluetoothDevice>> scanForPrinters({Duration timeout = const Duration(seconds: 12)}) async {
    try {
      ...
      subscription = FlutterBluePlus.scanResults.listen((scanResults) {
        for (var result in scanResults) {
          ...
          print('Dispositivo Bluetooth encontrado: ${device.name}, ID: $deviceId');
  ...
  Future<void> printTicket(Ventas venta) async {
    ...
    try {
      ...
      for (int i = 0; i < ticketData.length; i += chunkSize) {
        final end = (i + chunkSize < ticketData.length) ? i + chunkSize : ticketData.length;
      ...
      final finalCommands = Uint8List.fromList([0x0A, 0x0A, 0x0A, 0x1D, 0x56, 0x41, 0x00]);
      if (printCharacteristic.properties.writeWithoutResponse) {
